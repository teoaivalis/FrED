...
# Hyperparameters
BATCH_SIZE = 512 # Large batch size is crucial for contrastive learning!
EPOCHS = 100
LEARNING_RATE = 1e-3
TEMPERATURE = 0.07 # Scales the cosine similarities
SHARED_DIM = 512 # The size of your new unified space

# Setup DataLoader
#dataset = ArtEmbeddingDataset('image_embeddings.npy', 'graph_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_vitb32.npy', 'node2vec_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_vitb32.npy', 'graphsage_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_vitb32.npy', 'hashgnn_embeddings.npy')

#dataset = ArtEmbeddingDataset('image_embeddings_vit336.npy', 'node2vec_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_vit336.npy', 'graphsage_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_vit336.npy', 'hashgnn_embeddings.npy')

#dataset = ArtEmbeddingDataset('image_embeddings_rn5064.npy', 'node2vec_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_rn5064.npy', 'graphsage_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_rn5064.npy', 'hashgnn_embeddings.npy')

dataset = ArtEmbeddingDataset('image_embeddings_vit_g_14.npy', 'node2vec_embeddings.npy')
#dataset = ArtEmbeddingDataset('image_embeddings_vit_g_14.npy', 'hashgnn_embeddings.npy')
...
class ProjectionHead(nn.Module):
    ...
    def forward(self, x):
        x = self.mlp(x)
        # CRITICAL: L2 Normalize the output vector so its length is 1 (Cosine Similarity)
        return F.normalize(x, p=2, dim=1)

class MultiModalAlignmentModel(nn.Module):
    def __init__(self, img_in_dim=1024, graph_in_dim=512, shared_dim=512):
        super().__init__()
        # Since both your inputs are 512, the heads look identical
...
def info_nce_loss(z_img, z_graph, temperature=0.07):
    ...
    # Because vectors are L2 normalized, dot product == cosine similarity
    logits = torch.matmul(z_img, z_graph.T) / temperature
    
    # The correct matches are strictly on the diagonal (0 matches 0, 1 matches 1, etc.)
    N = logits.shape[0]
...
# ==========================================
#save_path = "multimodal_projection_heads_vitb32_hashgnn.pth"
#save_path = "multimodal_projection_heads_vit336_hashgnn.pth"
#save_path = "multimodal_projection_heads_rn5064_hashgnn.pth"
#save_path = "multimodal_projection_heads_dino_node2vec.pth"
