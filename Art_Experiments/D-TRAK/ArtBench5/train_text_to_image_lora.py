...
def main():
    ...
    print(empty_inputs.input_ids[0].size())
    ####
...
#         return {"pixel_values": pixel_values, "input_ids": input_ids}
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                for i in range(args.num_validation_images):
                    ...
                    if i==0:
                        images.append(
                        pipeline("a post impressionism painting", 
                        ...
                        pipeline("a post impressionism painting", 
                                 ...
                                 guidance_scale=7.5, # ???
                                 num_inference_steps=30, 
                                 generator=torch.Generator(device=accelerator.device).manual_seed(6)).images[0]
                    )
                    elif i==2:
                        images.append(
    ...
    if accelerator.is_main_process:
        unet = unet.to(torch.float32)
        # safe_serialization=False forces the use of pytorch_model.bin
